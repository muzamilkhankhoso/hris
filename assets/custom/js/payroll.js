...
$(document).ready(function(){
    ...
    fetch('http://innovative-apis.smrhr.com/calculateTax.php')
...
function showPayrollReportBank(){
    ...
    var bank_id=$('#sub_department_id').val();
    var company_id = m;
    ...
    if(validate == 0){
        ...
        $.ajax({
            ...
            type: "GET",
            data: {company_id:company_id,month_year:month_year,cheque_no:cheque_no,cheque_date:cheque_date,department_id:department_id,sub_department_id:sub_department_id,bank_id:bank_id,show_all:show_all},
            success:function(data) {
                $('#loader').html('');
                $('.employeeAttendenceReportSection').empty();
                $('.employeeAttendenceReportSection').append('<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">'+data+'</div>');
...
function viewEmployeePayrollForm(){
    ...
    if(validate == 0){
        ...
        if ($("#show_all").is(":checked")) {
            ...
            var datas = {show_all:show_all,slabs:slabs,emp_id:emp_id,payslip_month:payslip_month,m:m,sub_department:sub_department,department_id:department_id,emp_status:emp_status}
        }
        else{
            var datas = {emp_id:emp_id,slabs:slabs,payslip_month:payslip_month,m:m,sub_department:sub_department,department_id:department_id,emp_status:emp_status}
        }
...
function viewPayrollReport(){
    ...
    if(validate == 0){
        $("#employeeAttendenceReportSection").css({"display": "block"});
        if ($("#show_all").is(":checked")) {
            var data = {show_all:show_all,m:m,payslip_month:payslip_month,department_id:department_id,sub_department_id:sub_department_id,emp_status:emp_status}
        }
        else{
            var data = {m:m,payslip_month:payslip_month,department_id:department_id,sub_department_id:sub_department_id,emp_id:emp_id,emp_status:emp_status}
        }
...
function viewConcileReport(){
    ...
    if(validate == 0){
        ...
        var data = {m:m,month_to:month_to,month_from:month_from,department_id:department_id,sub_department_id:sub_department_id,show_all:show_all}
