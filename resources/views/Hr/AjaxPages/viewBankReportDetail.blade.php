...
if($show_all== 1){
    $payslip = DB::table('payslip')
        ...
        ->select('employee.emp_name', 'employee.account_title', 'employee.bank_account','payslip.net_salary')
        ->where([["payslip.month","=",$explodeMonthYear[1]],["payslip.year","=",$explodeMonthYear[0]],['payslip.payment_mode','=','Transfer'],['employee.status','=','1'],['employee.bank_id','=',$bank_id]]);


    $total_net = DB::table('payslip')
        ->join('employee', 'employee.emp_id', '=', 'payslip.emp_id')
        ->select(DB::raw('SUM(payslip.net_salary) As total_net'))
        ->where([["payslip.month","=",$explodeMonthYear[1]],["payslip.year","=",$explodeMonthYear[0]],['payslip.payment_mode','=','Transfer'],['employee.status','=','1'],['employee.bank_id','=',$bank_id]])->get();


    $total_net = $total_net[0]->total_net;
}
elseif(($department_id != 0 && $department_id != '') && ($sub_department_id != 0 && $sub_department_id != '')){
    $payslip = DB::table('payslip')
        ->join('employee', 'employee.emp_id', '=', 'payslip.emp_id')
        ->select('employee.emp_name', 'employee.account_title', 'employee.bank_account','payslip.net_salary')
        ->whereIn('employee.emp_department_id',$department_id)
        ->whereIn('employee.emp_sub_department_id',$sub_department_id)
        ->where([["payslip.month","=",$explodeMonthYear[1]],["payslip.year","=",$explodeMonthYear[0]],['payslip.payment_mode','=','Transfer'],['employee.status','=','1'],['employee.bank_id','=',$bank_id]]);
    ...
    <div class="panel-body">
        <div class="row">
            <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <h4>
                            <b>The Manager <br>
                                {{ $bank_name }}<br>
                                @if($bank_id == 1) Progressive Plaza Branch @else  __________________ Branch  @endif <br>
                                @if($bank_id == 1) Beaumont Road @else  _______________ Road  @endif<br>
                                Karachi
                            </b>
                        </h4>
                    </div>
                ...
                <div class="row">
                    <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                        <h4> <b> @if($bank_id == 1) ACCOUNT NO. 20311-714-118717 @else  ACCOUNT NO._____________  @endif </b>   </h4>
                    </div>
                </div>
                    ...
                    <table class="table table-responsive table-bordered table-condensed" id="regionWisePayrollReport">
                        ...
                        <tr>
                            ...
                            <td class="text-center">
                                <b><?php echo number_format($payslip->sum('net_salary'),0);?></b>
                            </td>
                        </tr>
