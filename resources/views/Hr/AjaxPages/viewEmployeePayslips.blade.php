                        ...
                        @if($payslipData->count() > 0)
                            @foreach($payslipData->get() as $value)
                                <?php
                                     ...
                                     $email = Employee::select('professional_email','emp_name')->where('emp_id',$value->emp_id);
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($email->value('professional_email') != '')
                                            {{$email->value('professional_email') }}
                                        @endif
                                    </td>
                                    <td class="text-center">
                                        <a target="_blank" href="{{ url('/') }}/hr/viewPayslipPdf/{{ Input::get('m') }}/{{ $value->month }}/{{ $value->year }}/{{ $value->emp_id }}">
                                            View
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        <a target="_blank" href="{{ url('/') }}/hr/downloadPayslipPdf/{{ Input::get('m') }}/{{ $value->month }}/{{ $value->year }}/{{ $value->emp_id }}">
                                            <img src="{{ url('/') }}/assets/images/pdf.png" height="20" width="20">
                                        </a>
                                    </td>
                                    <td class="text-center">
                                        @if($email->value('professional_email') != '')
                                            <input type="checkbox" checked class="ads_Checkbox " name="check_list[]" value="{{ $email->value('professional_email').'<>'.$value->emp_id }}">
                                        @endif
                                    </td>
