                ...
                <div class="table-responsive">
                    <table class="table table-bordered sf-table-list table-hover" id="EmployIdCardRequestList">
                        @if($hr_warning_letter->count() > 0)
                            ...
                            <tbody>
                            <?php $counter = 1;?>
                            @foreach($hr_warning_letter->get() as  $row)

                                <tr>
                                    <td class="text-center"><input type="hidden" value="{{$m}}" name="company_id">{{ $counter++ }}</td>
                                    <td class="text-center">{{ $row->emp_id }}</td>
                                    <td class="text-center">{{ HrHelper::getCompanyTableValueByIdAndColumn($m, 'employee', 'emp_name', $row->emp_id, 'emp_id') }} </td>
                                            ...
                                            <ul class="dropdown-menu" role="menu" aria-labelledby="menu1">
                                                @if(in_array('view', $operation_rights2))
                                                    <li role="presentation">
                                                        <a class="delete-modal btn"  href="<?= url("/hdc/viewHrWarningLetter/{$row->id}/{$m}?pageType=viewlist&&parentCode=27&&m={$m}")?>" target="_blank">
                                                            View
                                                        </a>
                                                    </li>
                                                @endif
