            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body" id="PrintEmployeeList">
                        <div class="row">
                            <div class="col-sm-8">
                                <h4 class="card-title">View Employee Grades List</h4>
                            </div>
                        ...
                        <div class="table-responsive" id="PrintMaritalStatusList">
                            <table class="table table-sm mb-0 table-bordered table-striped table-hover" id="MaritalStatusList">
                                ...
                                <th class="text-center">Employee Grades Type</th>
                                ...
                                <tbody>
                                <?php $counter = 1;?>
                                @foreach($EmployeeGrades as $key => $y)
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            @if(in_array('edit', $operation_rights))
                                                <button class="edit-modal btn btn-sm btn-info" onclick="showMasterTableEditModel('hr/editEmployeeGradesDetailForm','<?php echo $y->id ?>','Employee Grades Edit Detail Form','<?php echo $m?>')">
                                                    <span class="fas fa-edit"></span>
                                                </button>
                                            @endif
                                            @if(in_array('repost', $operation_rights))
                                                @if($y->status == 2)
                                                    <button class="delete-modal btn btn-sm btn-primary" onclick="repostMasterTableRecords('<?php echo $y->id ?>','grades')">
                                            ...
                                            @if(in_array('delete', $operation_rights))
                                                @if($y->status == 1)
                                                    <button class="delete-modal btn btn-sm btn-danger" onclick="deleteRowMasterTable('<?php echo $y->id ?>','grades')">
