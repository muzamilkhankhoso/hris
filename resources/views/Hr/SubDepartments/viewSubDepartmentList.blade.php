            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-body" id="PrintEmployeeList">
                        <div class="row">
                            <div class="col-sm-8">
                                <h4 class="card-title">View Sub Department List</h4>
                            </div>
                        ...
                        <div class="row">
                            <div class="col-lg-12 col-md-12 col-sm-12col-xs-12">
                                <div class="table-responsive">
                                    ...
                                    <table class="table table-sm mb-0 table-bordered table-striped table-hover" id="SubDepartmentList">
                                        <thead>
                                        ...
                                        @foreach($SubDepartments as $key => $y)
                                            <?php $departmentName = DB::table('department')->where('id','=', $y->department_id); ?>
                                            <tr>
                                                <td class="text-center"><?php echo $counter++;?></td>
                                                <td>
                                                    <?php
                                                    if($departmentName->count() > 0){
                                                        echo $departmentName->value('department_name');
                                                    }
                                                    else{
                                                        echo '--';
                                                    }
                                                ...
                                                <td class="text-center hidden-print">
                                                    @if(in_array('edit',$operation_rights))
                                                        <button class="edit-modal btn btn-sm btn-info" onclick="showMasterTableEditModel('hr/editSubDepartmentForm','<?php echo $y->id ?>','Sub Department Edit Detail Form','<?php echo $m?>')">
                                                            <span class="fas fa-edit"></span>
                                                        </button>
                                                    @endif
                                                    @if(in_array('repost',$operation_rights))
                                                        @if($y->status == 2)
                                                            <button class="delete-modal btn btn-sm btn-primary" onclick="repostMasterTableRecords('<?php echo $y->id ?>','sub_department')">
                                                    ...
                                                    @if(in_array('delete',$operation_rights))
                                                        @if($y->status == 1)
                                                            <button class="delete-modal btn btn-sm btn-danger" onclick="deleteRowMasterTable('<?php echo $y->id ?>','sub_department')">
